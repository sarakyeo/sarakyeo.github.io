...
1) Find the journal article below and download the data and codebook from OSF (link available in paper). In this assignment, you will recreate three of the variables (`mirth`, `need for humor`, and `engagement intentions`) from the article. We will use these variables to examine some differences and relationships.
...
3) Read the data into R. Answer the following questions as comments in your R script.
...
8) Next, we will test whether the experimental conditions had an effect on mirth for the three issues. 
    ...
    e) Re-create the individual graphs in Figure 3 in Yeo *et al.* (2023).
...
Submit your R script (named `Assignment-#_FirstName-LastName.R`) to Canvas.
