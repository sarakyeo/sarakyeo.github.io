...
Let's try out some commands. Open RStudio and start a new R script. If you need a reminder, review section \@ref(tour).

To start, let's execute a simple command. In the **Editor**, type:
```
4 + 8
```
...
You will see the result of the command you just ran in the **Console** window. Note that the commands in the Console panel are preceded by `>`.
...
Let's say that we want to save the result of `2 * 10` to an object and call it `catpuke`. In the **Editor** window, type and run the following R code:
...
```{r kittens-env, echo = FALSE, fig.align="center", fig.cap="The Environment panel after you run the R code to create the object, `kittens`."}
