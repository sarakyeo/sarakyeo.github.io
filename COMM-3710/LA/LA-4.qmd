...
# 6a: Number of performances that occurred this week.
# 6b: Statistics.Gross Potential

```

7)  In the next step, we will learn to use the pipe operator. The pipe operator, `|>`, is used to perform sequential functions in R. It is a base R operator (built into R itself, no package required) and is also compatible with the `tidyverse` package. You can also think of the pipe operator, `|>`, as "then." If we were to use `|>` to describe a daily routine, it might look something like this:

Note that this is not real, runnable R code — it's just an analogy to help you understand what the pipe operator does. Don't try to run it in your Console.
...
# 7a: Show.Name
# 7b: The numeric month that this performance's week ended in (1 = January, 2 = February, etc.).
# 7c: Statistics.Attendance
...
# 9a: Min = 5,737 people
# 9b: Max = 16,319 people
# 9c: Mean = 13,477 people
# 9d: The Lion King is more successful.

```

10) So far, we've only filtered on one condition at a time (e.g., one show name). Sometimes we want to filter on *two* conditions at once — for example, cases that match a particular theater *and* a particular show type. To do this, we can add a second `filter()` step to our pipe, like this:
...
bway |> # Start with the broadway data frame
        filter(Show.Theater == "Studio 54") |> # First, keep only Studio 54 cases
        filter(Show.Type == "Play") |> # Then, keep only Play cases from what's left
...
Each `filter()` step narrows down the data further, so by the time we get to `summarise()`, we're only looking at cases that meet *both* conditions.
...
a)  Among shows in the `Majestic` theater that are labeled as `Musical`, what is the minimum, maximum, and mean "Gross" in dollars?
b)  Among shows in the `Vivian Beaumont` theater that are `Musical`, what is the minimum, maximum, and mean "Gross" in dollars?
...
bway |> # Start with the broadway data frame
        filter(Show.Theater == "Majestic") |> # Select only Majestic cases
...
# 10a: min = 185490 USD, max = 1843296 USD, mean = 750724 USD
# 10b: min = 67578 USD, max = 1247155 USD, mean = 561461 USD
# 10c: Majestic is more successful.
