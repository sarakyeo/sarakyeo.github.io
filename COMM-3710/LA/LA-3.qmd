...
- Create sections in your R script.
...
2) Make a section of code titled "Installing and loading packages." Under this heading, write some pseudocode in preparation to install and load the following packages:
...
:::callout-tip
Pseudocode is the plain English version of what you want your R code to do. Pseudocode describes the steps you are taking in R. It should be written as comments in your R script and should make sense to you. Your pseudocode is a tool that helps you decide which R function or command you need to accomplish a given task.
:::
...
4) Load the packages. The function to load the packages is `library()`. To load the `tidyverse` package, type `library(tidyverse)` in your script and run it. Note that for each new Project you create in Your Workspace on Posit Cloud, you will have to install and load any necessary packages again. If you are using the same Project, you may not need to load the packages each time you open the project.

# Entering Data Manually

In the next step, we will enter data into R manually using the `c()` function, which is short for `combine()`. Before we do so, let's remind ourselves about the function we will be using.

1) Create a new section of code where you can explore and learn about the `c()` function.

2) To read the documentation for any function, you can add `?` in front of the command and run it. Alternatively, you can type `?<function>` in the Console. This will pull up the documentation (including examples at the very end of the documentation) in the Help tab in the bottom right pane. For example, if I wanted to read the documentation for the `library()` function, I would run the command: `?library()`. Copy and paste the description from the Help pane for the `c()` function in your R script as a comment.

:::callout-tip
The documentation may not seem helpful at first glance, but once you become more familiar with R and its commands, the Help documentation becomes a useful resource. Often, the most helpful part of the documentation is the section of examples--scroll down to the bottom of the documentation to check these out.
:::

3) Enter the data in the table below as four (4) objects called `Aname`, `Ateam`, `PPG` (points per game), and `REB` (rebounds per game). There are two data types in the data table, numeric and character. Remember to used the `<-` and `c()` to assign data to objects.

:::callout-tip
Some helpful code to assign numeric data to objects: `objectname <- c(#, #, #)`
Some helpful code to assign character/text data to objects: `objectname <- c("text", "text", "text")`
:::
...
:::callout-tip
A template for using `tibble()` to create a data frame: `dataframename <- tibble(objectname1, objectname2, objectname3)`
:::
...
Install and load the `learnr` package in your current session of R (review the instructions above if you need a reminder of how to install and load packages). If you are asked to install packages or dependencies, do so. In the Tutorial tab (in the top right with the Environment pane), complete the **Data Basics** tutorial.
...
- Reopen your project and ensure that the Environment is clean.
