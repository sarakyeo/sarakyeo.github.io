---
title: "Practice Working with Data"
format:
   pdf:
...
4) Load the packages using the `library()` function. If you need a reminder of how to do this, refer to a previous lab assignment.

# Entering (More) Data Manually

1) Enter the data in the table below as five separate objects called `educ`, `polviews`, `gunlaw` (whether the respondent supports a law which requires a person to obtain a permit before they can buy a gun), `owngun` (whether the respondent owns a gun), and `numkids` (number of children in their household). There are two data types in the data table, numeric and character.
...
::: callout-tip
Each column in the dataframe is also known as a **variable**. In the dataframe you just created, there are five (5) variables: `educ`, `polviews`, `gunlaw`, `owngun`, and `numkids`.
:::
...
```
dataframe |> 
        ggplot(aes(x = gunlaw, fill = owngun)) +
        geom_bar()

```

The figure should look like the one below. As a comment in your R script, describe the figure. In other words, what do you notice about the relationship between the two variables, `owngun` and `gunlaw`?
...
- Reopen your project and ensure the Environment is clean.
