...
> **You should follow these instructions for preparation, naming, and saving of your R script for *all* of your individual lab assignments.** 
...
Use this [template](https://utah.instructure.com/files/120006973/download?download_frd=1) to help organize your R script.
...
Then, write the R command to set your working directory.
...
##  Instructions
In the next step, we will enter data into R manually using the `c()` function, which is short for `concatenate()`. Before we do so, let's learn about the function we will be using.
...
Then, write the R command (`?c`) that opens the documentation in your R script. 

Describe the `c()` function from the documentation as a comment under your `?c` command. You can copy and paste the text under "Description" from the documentation, but I encourage you to add your own description so that you understand what the function is doing.
...
## Hint
To call an object, type the name of the object in the Console and press Enter.
...
Then, write psedocode and R code to check the class of each column in your data frame. Note the class of each column of your data frame in a comment.
...
Install and load the `learnr` package in your current session of R. In the **Tutorial** panel (usually top right with the **Environment** panel), scroll down to "Data basics" and complete this tutorial.

Include the answers to the questions below in your R script as comments. Clearly label your answers with the question number (e.g., 10a, 10b).
...
d) Which types of variables does `flights` contain? List all that apply.
...
## Examples and R script template
A template for your R script can be found [here](https://utah.instructure.com/files/120006973/download?download_frd=1).

And an example of how to write pseudocode can be seen in this gif.
![](LA-3_example.gif)