...
# Step 6
## Question
We will now learn how to use the `mutate()` function to create a new variable from existing ones. Assume we want to sum each respondent's score of `daysanysub` and `dayslink`.

To do so, we need to first check the frequency distributions of each variable. In general, it is good practice to examine the frequency distributions of all variables before working with them.
